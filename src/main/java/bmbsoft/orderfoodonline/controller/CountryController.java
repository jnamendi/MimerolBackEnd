...
public class CountryController extends BaseController {
	...
	@RequestMapping(value = "/api/country/getAll", method = RequestMethod.GET)
	public ResponseEntity<?> getAll() {
		ResponseGetPaging responseGetPaging = new ResponseGetPaging();
		List<CountryViewModel> vm = null;
		try {
			vm = this.countryService.getAll();
	...
	@RequestMapping(value = "/api/country/getById/{id}", method = RequestMethod.GET)
	public ResponseEntity<?> getById(@PathVariable long id) {
		ResponseGet responseGet = new ResponseGet();
		try {
			CountryViewModel c = countryService.getById(id);
			if (null == c) {
	...
	@RequestMapping(value = "/admin/country/save", method = RequestMethod.POST)
	public ResponseEntity<?> create(@RequestBody CountryViewModel model) {
		ResponseGet responseGet = new ResponseGet();
		try {
			if (countryService.create(model) == true) {
				responseGet.setStatus(0);
	...
	@RequestMapping(value = "/admin/country/save/{id}", method = RequestMethod.PUT)
	public ResponseEntity<?> update(@PathVariable long id, @RequestBody CountryViewModel model) {
		ResponseGet responseGet = new ResponseGet();
		try {
			CountryViewModel c = this.countryService.getById(id);
			if (null == c) {
				responseGet.setStatus(0);
				responseGet.setMessage("country not exists");
				responseGet.setErrorType(Constant.ErrorTypeCommon.NOT_FOUND_ITEM);
			...
			if (countryService.update(model) == true) {
				responseGet.setStatus(0);
	...
	@RequestMapping(value = "/admin/country/delete/{id}", method = RequestMethod.DELETE)
	public ResponseEntity<?> delete(@PathVariable long id) {
		ResponseGet responseGet = new ResponseGet();
		try {
			Country c = this.countryService.getBaseById(id);
			if (null == c) {
				responseGet.setStatus(0);
				responseGet.setMessage("country not exists");
				responseGet.setErrorType(Constant.ErrorTypeCommon.NOT_FOUND_ITEM);
	...
	@RequestMapping(value = "/admin/country/deleteMany", method = RequestMethod.POST)
	public ResponseEntity<?> deleteMany(@RequestBody DeleteManyRequest cr) {
		...
		try {
			...
			for (int i = 0; i < cr.getIds().length; i++) {
				...
				Country c = countryService.getBaseById(id);
				if (c != null) { 
